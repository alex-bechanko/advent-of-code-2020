...
import (
	...
	"github.com/alex-bechanko/advent-of-code-2020/pkg/day14"
	"github.com/spf13/cobra"
)

func init() {

	var defaultInput = fmt.Sprintf("inputs/day14.txt")
	var fileInput string

	var day14Cmd = &cobra.Command{
		Use:   "day14",
		Short: "Compute day 14 solutions",
		Long:  "Computes part1 and part2 solutions for day 14 of Advent of Code 2020",
		Run: func(cmd *cobra.Command, args []string) {
			data1, err := day14.Parse(fileInput)
			if err != nil {
				fmt.Fprintf(os.Stderr, "Error parsing data file: %v\n", err)
				os.Exit(1)
			}

			data2, err := day14.Parse(fileInput)
			if err != nil {
				fmt.Fprintf(os.Stderr, "Error parsing data file: %v\n", err)
				os.Exit(1)
			}

			soln1, err := day14.Solution1(data1)
			if err != nil {
				fmt.Fprintf(os.Stderr, "Error computing solution 1: %v", err)
			}

			soln2, err := day14.Solution2(data2)
			if err != nil {
	...
	day14Cmd.Flags().StringVarP(&fileInput, "input", "i", defaultInput, "Path to the input data")
