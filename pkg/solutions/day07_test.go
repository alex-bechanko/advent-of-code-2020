...
func Test_Day07Parse(t *testing.T) {
	...
	data, err := Day07Parse(testPath)
	assert.NoError(t, err)
	assert.Equal(t, expected, data, "data does not match expected")
}

func Test_Day07Solution01(t *testing.T) {
	data := BagGraph{
		Nodes: map[string]bool{
			"bright white": true,
			"dark olive":   true,
	...
	expected := "4"
	actual, err := Day07Solution01(data)
	assert.NoError(t, err)
	assert.Equal(t, expected, actual)

}

func Test_Day07Solution02(t *testing.T) {
	data := BagGraph{
		Nodes: map[string]bool{
			"bright white": true,
			"dark olive":   true,
	...
	expected := "32"
	actual, err := Day07Solution02(data)
	assert.NoError(t, err)
	assert.Equal(t, expected, actual)
}
